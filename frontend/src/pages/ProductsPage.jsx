...
const ImageLightbox = ({ images, startIndex, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center" onClick={onClose}>
      <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center">
        <X className="w-5 h-5 text-white" />
      </button>
      <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white/50 text-xs font-mono">{current + 1} / {images.length}</div>
      {images.length > 1 && (
        <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 w-11 h-11 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center">
      ...
      {images.length > 1 && (
        <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 w-11 h-11 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {selectedProduct && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-3 md:p-6"
            onClick={() => setSelectedProduct(null)}>
            <div className="bg-white rounded-2xl w-full max-h-[92vh] overflow-y-auto"
              ...
              <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between z-10">
                <div>
                  <h2 className="text-lg md:text-xl font-bold text-slate-800 leading-tight">{selectedProduct.name}</h2>
                  <span className="text-xs text-slate-400">{selectedProduct.categoryName} · {selectedProduct.id}</span>
                </div>
                <button onClick={() => setSelectedProduct(null)}
                  className="w-8 h-8 rounded-full hover:bg-slate-100 flex items-center justify-center transition-colors flex-shrink-0">
                  <X className="w-5 h-5 text-slate-500" />
                </button>
              </div>

              {/* ── TOP HALF: Images (left) + Tabs (right) ── */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-b border-slate-100">
                ...
                <div className="p-4 border-b md:border-b-0 md:border-r border-slate-100">
                  {/* Main image */}
                  <div className="rounded-xl overflow-hidden bg-slate-50 cursor-zoom-in relative group mb-3"
                    style={{ aspectRatio: '1', border: '1px solid #f0f0f0' }}
                    onClick={() => setLightbox({ images, index: activeImg })}>
                    <img src={images[activeImg]} alt={selectedProduct.name}
                      className="w-full h-full object-contain p-3 group-hover:scale-105 transition-transform duration-300"
                      onError={e => { e.target.src = 'https://via.placeholder.com/400?text=No+Image'; }} />
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/05 transition-colors flex items-center justify-center">
                      <span className="opacity-0 group-hover:opacity-100 bg-black/50 text-white text-xs px-2 py-1 rounded-full transition-opacity">🔍 Zoom</span>
                    </div>
                  </div>
                  {/* Thumbnails */}
                  {images.length > 1 && (
                    <div className="flex gap-2 flex-wrap">
                      {images.map((img, i) => (
                        <div key={i} onClick={() => setActiveImg(i)}
                          className="w-14 h-14 rounded-lg overflow-hidden cursor-pointer transition-all bg-slate-50"
                ...
                {/* Tabs: Features / Spec / Glass / Finishes */}
                <div className="p-4">
                  <div className="flex border-b border-slate-200 mb-4 gap-1 overflow-x-auto">
                    {['features', 'spec', 'glass', 'finishes'].map(tab => (
                      <button key={tab} onClick={() => setActiveTab(tab)}
                        className="pb-2 px-3 text-xs font-bold capitalize whitespace-nowrap transition-all"
                        style={{
                    ...
                    <ul className="space-y-2">
                      {data?.commonData?.features?.map((item, i) => (
                        <li key={i} className="flex items-center gap-2 text-slate-700 text-sm">
                          <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: '#f59e0b' }}></span>{item}
                        </li>
                      ))}
                    </ul>
                  )}
                    ...
                    <ul className="space-y-2">
                      {data?.commonData?.specifications?.map((item, i) => (
                        <li key={i} className="flex items-start gap-2 text-slate-700 text-sm">
                          <span className="w-2 h-2 rounded-full flex-shrink-0 mt-1.5" style={{ background: '#f59e0b' }}></span>{item}
                        </li>
                      ))}
                    </ul>
                  )}
              ...
              {/* ── BOTTOM HALF: Description full width ── */}
              <div className="p-5">
                  ...
                  <div className="rounded-xl p-4 mb-4" style={{ background: '#fffbf0', border: '1px solid #fde68a' }}>
                    ...
                    {hasInfo && (
                      <div className="grid grid-cols-2 gap-x-6 gap-y-2 mb-3">
                        {parsed.modelName && (
                          <div>
                            <span className="text-xs text-slate-500 block">Model Name</span>
                            <span className="text-sm font-bold text-slate-800">{parsed.modelName}</span>
                          </div>
                        )}
                        {parsed.weight && (
                          <div>
                            <span className="text-xs text-slate-500 block">Weight</span>
                            <span className="text-sm font-bold text-slate-800">{parsed.weight}</span>
                          </div>
                        )}
                        {parsed.length && (
                          <div>
                            <span className="text-xs text-slate-500 block">Length</span>
                            <span className="text-sm font-bold text-slate-800">{parsed.length}</span>
                          </div>
                        )}
                        {parsed.profileSize && (
                          <div>
                            <span className="text-xs text-slate-500 block">Profile Size</span>
                            <span className="text-sm font-bold text-slate-800">{parsed.profileSize}</span>
                          </div>
                        )}
                      </div>
                    )}
                      ...
                      <ul className="space-y-1.5">
                        {parsed.bullets.map((b, i) => (
                          <li key={i} className="flex items-start gap-2 text-slate-600 text-sm">
                            <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-1.5" style={{ background: '#f59e0b' }}></span>
                            {b}
                          </li>
                        ))}
                      </ul>
                ...
                <div className="flex gap-3">
                  ...
                  <Link to="/contact"
                    className="flex-1 flex items-center justify-center py-3 rounded-xl font-bold text-sm text-white transition-all hover:opacity-90"
