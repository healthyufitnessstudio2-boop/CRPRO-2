...
// ─── Helper: Description se Model Name, Weight, Length, Profile Size parse karo
    ...
    fetch('/data/products.json')
      ...
      .then(data => {
        ...
        if (categoryId) {
          ...
          const prodsWithCat = (cat?.products || []).map(p => ({ ...p, categoryName: cat.name }));
        ...
        <div className="w-full px-4">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-4">
            {filteredProducts.map((product, idx) => (
              <div
                key={product.id}
                onClick={() => { setSelectedProduct(product); setActiveTab('features'); }}
                            ...
                            <tbody>
                              {parsed.modelName && (
                                <tr className="border-b border-amber-100">
                                  <td className="py-1.5 text-slate-500 font-medium w-2/5">Model Name</td>
                                  <td className="py-1.5 text-slate-800 font-bold">: &nbsp;{parsed.modelName}</td>
                                </tr>
                              )}
                              {parsed.weight && (
                                <tr className="border-b border-amber-100">
                                  <td className="py-1.5 text-slate-500 font-medium">Weight</td>
                                  <td className="py-1.5 text-slate-800 font-bold">: &nbsp;{parsed.weight}</td>
                                </tr>
                              )}
                              {parsed.length && (
                                <tr className="border-b border-amber-100">
                                  <td className="py-1.5 text-slate-500 font-medium">Length</td>
                                  <td className="py-1.5 text-slate-800 font-bold">: &nbsp;{parsed.length}</td>
                                </tr>
                              )}
                              {parsed.profileSize && (
                                <tr>
                                  <td className="py-1.5 text-slate-500 font-medium">Profile Size</td>
                                  <td className="py-1.5 text-slate-800 font-bold">: &nbsp;{parsed.profileSize}</td>
                                </tr>
                              )}
                            </tbody>
                          </table>
